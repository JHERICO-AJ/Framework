    ...
    with open(path, "w", encoding="utf-8") as fh:
        ...
        if stats["fallas"]:
            ...
            for f in stats["fallas"][:15]:
                fh.write(f"  {f}\n")
        if stats.get("capturas"):
            fh.write("\nEvidencia (screenshots):\n")
            for c in stats["capturas"][:15]:
                fh.write(f"  {c}\n")
...
def run():
    ...
    stats = {"calc_ok": 0, "calc_fail": 0, "calc_desc": 0,
             "pant_ok": 0, "pant_fail": 0, "pant_none": 0,
             "fallas": [], "capturas": []}
    last_omni = 0.0
    venia_falla = False        # para capturar SOLO al entrar en falla (flanco)
    try:
        while True:
            now = time.time()
            try:
            ...
            if now - last_omni >= OMNIOPS_EVERY_S:
                ...
                try:
                    ...
                    if api_val is None or tep is None:
                        ...
                        # evidencia visual: captura SOLO al ENTRAR en falla
                        # (flanco) -> una por episodio, no una por segundo.
                        hay_falla = "FALLA" in (calc.strip(), pant.strip())
                        if CAPTURAR_FALLAS and hay_falla and not venia_falla:
                            quien = "_".join(q for q, c in (
                                ("calculo", calc.strip() == "FALLA"),
                                ("pantalla", pant.strip() == "FALLA")) if c)
                            sello = datetime.datetime.now().strftime("%Y%m%d_%H%M%S")
                            cap = ui.captura(f"{CAPTURAS_DIR}/falla_{sello}_{quien}.png")
                            if cap:
                                stats["capturas"].append(cap)
                                print(f"           evidencia: {cap}")
                        venia_falla = hay_falla
