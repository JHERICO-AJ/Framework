...
from ui.pages.monitoring_page import MonitoringPage, MONITORING_URL, parse_kw
...
class UiSession:
    ...
    def captura(self, path):
        """Guarda un screenshot del dashboard como EVIDENCIA. Nunca rompe el
        monitor: si algo falla, devuelve None; si sale bien, devuelve la ruta."""
        import os
        try:
            os.makedirs(os.path.dirname(path) or ".", exist_ok=True)
            self.monitoring.captura(path)
            return path
        except Exception:
            return None
