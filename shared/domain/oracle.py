...
from shared.datasource.modbus_source import signed16, LectorModbus, LectorFalso
...
def causa_de(alarma, reader):
    ...
    try:
        ...
        if layer == "telemetry":
            ...
            t = TELEMETRY.get(alarma["alarm_rule_id"])
            if not t:
                return NV, "telemetría sin mapear (falta dirección del Model 803)"
            raw = reader.leer_registro(t["field"])
            if t["tipo"] == "int16":
                raw = signed16(raw)
            sf = signed16(reader.leer_registro(t["sf"]))
            valor = raw * (10 ** sf)
            op = _OPERADORES[t["op"]]
            aprox = " (aprox)" if t.get("aprox") else ""
            return bool(op(valor, t["limite"])), (
                f"{valor:.2f} {t['op']} {t['limite']}{aprox}")
...
def _self_check():
    ...
    reader = LectorFalso({
        ...
        # telemetría: ID 16 Rack High Temp (StrModTmpMax 10173, SF 10156)
        10173: 30000, 10156: 0,          # 30000 * 10^0 = 30000 °C  > 58 -> SÍ
    })
    ...
    esperado = {
        ...
        16: SI,   # telemetry: 30000°C > 58 -> presente
