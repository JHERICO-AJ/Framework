...
class Overrides:
    """Qué hacerle a cada registro: OR (prender), CLEAR (apagar), REPLACE (fijar valor)."""

    def __init__(self):
        self.set = {}      # {reg16: máscara a prender}
        self.clear = {}    # {reg16: máscara a apagar}
        self.replace = {}  # {reg16: valor a fijar} (telemetría)
...
def overrides_para(rule_ids):
    ...
    for rid in rule_ids:
        ...
        if "evt1_802_bits" in inj:
            ...
            from shared.domain.telemetry_map import TELEMETRY, raw_extremo
            t = TELEMETRY.get(rid)
            if t is None:
                ov.no_soportado.append((rid, "telemetría sin mapear (falta dirección 803/103)"))
            else:
                ov._fijar(t["field"], raw_extremo(t["dir"], t["tipo"]))
        else:
...
def leer_estado():
    """Lo que el proxy aplica: (set {reg:mask}, clear {reg:mask}, replace {reg:valor}).
    ...
    try:
        ...
        rep = {int(r): int(m) for r, m in data.get("replace", {}).items()}
        return pon, qui, rep
    except Exception:
        return {}, {}, {}
...
def _self_check():
    ...
    # telemetría ahora soportada: produce un REPLACE (valor extremo)
    chequeo("ID 16 telemetría -> replace{10173:30000}", overrides_para([16]).replace, {10173: 30000})
